...
// Curated homepage reviews — real Etsy customer photos & text, 17 entries
export const HOMEPAGE_REVIEWS: ProductReview[] = [
  {
    author: 'Pam',
    rating: 5,
...
// Legacy static reviews kept for any component still referencing REVIEWS
export const REVIEWS = HOMEPAGE_REVIEWS.map((r) => ({
  stars: '★'.repeat(r.rating) + '☆'.repeat(5 - r.rating),
  quote: r.body,
  name: r.author,
  product: `${r.product} · ${r.created_at}`,
}));
